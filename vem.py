...
Original file is located at
    https://colab.research.google.com/drive/1_xfaRk_UxMSDerNd9bDme5tcOsAX0GaR
"""
...
def quadratic_polynomials():
    return [[0, 0], [1, 0], [0, 1], [1, 1], [0.5, 0], [0, 0.5], [0.5, 0.5]]
...
def vem(mesh_file, rhs, boundary_condition):
    ...
    n_polys = 7  # Here We have changed the number of polynomials to 7 for quadratic

    K = np.zeros((n_dofs, n_dofs))
    F = np.zeros(n_dofs)
    u = np.zeros(n_dofs)

    quadratic_polys = quadratic_polynomials()
    ...
    for el_id in range(elements.shape[0]):
        ...
        for vertex_id in range(n_sides):
            ...
            for poly_id in range(1, n_polys):  # Here we are looping over non-constant polynomials
                poly_degree = quadratic_polys[poly_id]
